...
survival_data <- merged_data_DCIS %>%
  select(Patient_ID, Age.at.Diagnosis..years., Progression.or.Recurrence, Days.to.Last.Known.Disease.Status, Signature_Score, Signature_Group)
...
sum(is.na(survival_data$Signature_Score))
sum(is.na(survival_data$Signature_Group))
...
survival_data <- survival_data %>%
  filter(!is.na(Age.at.Diagnosis..years.) & !is.na(Progression.or.Recurrence) & !is.na(Days.to.Last.Known.Disease.Status) 
         & !is.na(Event) & !is.na(Signature_Score) & !is.na(Signature_Group))
...
table(survival_data$Progression.or.Recurrence, survival_data$Event)
...
write.csv(cox_DCIS_results, file = "cox_DCIS_results_signature_score.csv", row.names = FALSE)
